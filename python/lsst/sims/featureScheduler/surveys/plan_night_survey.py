...
class Plan_ahead_survey(Blob_survey):
    ...
    def check_night(self, conditions):
        ...
        if area > self.minimum_sky_area:
            ...
            mean_RA = np.arctan2(np.mean(np.sin(conditions.ra[pix_to_obs])), np.mean(np.cos(conditions.ra[pix_to_obs])))
            if mean_RA < 0:
                mean_RA += 2.*np.pi
        ...
        if self._check_feasibility(conditions):
            ...
            if self.scheduled_obs is not None:
                ...
                if (np.abs(delta_mjd) < self.delta_mjd_tol) & (self.scheduled_obs is not None):
                    ...
                    self.reward = super(Plan_ahead_survey, self).calc_reward_function(conditions)
